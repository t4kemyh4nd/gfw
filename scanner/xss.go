...
func (s XSSscanner) getGETValues(req *http.Request) map[string][]string {
...
func (s XSSscanner) removeGETMalChars(req *http.Request) bool {
	...
	for keys, values := range queryMap {
		for index, param := range values {
			...
			//if above filter is bypassed somehow
			for _, m := range malChars {
				if strings.Contains(param, m) {
					param = strings.ReplaceAll(param, m, "")
				}
			}
			//if bypassed again
			flag = !xssRegex.MatchString(param)
			values[index] = param
		}
...
func ScanForXSS(req *http.Request) bool {
	...
	return xsscanner.removeGETMalChars(req)
}
